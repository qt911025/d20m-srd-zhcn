...
拔出隐藏武器会比从正常的装具中拔出更困难，一般需要一个攻击动作。将武器藏在更容易拔出的位置会让武器更难于隐藏。

### 手上功夫检定

要隐藏武器或其他物品，应做一个手上功夫检定。 角色在公开露面前隐藏物品通常可以取10，除非角色很匆忙、隐藏时能被其他人看到、或在其他不寻常的限制下。此形式的手上功夫无需受训即可使用，但角色必须取10。

### 尺寸和隐藏

物品的大小影响检定结果，如`表：隐藏武器与物品`所示。使用的装具类型或穿着的衣物，以及任何让武器易于拔出的方式，都会影响检定。
...
侦察DC=目标的手上功夫技能调整值（包括`表：隐藏武器与物品`的调整值）+10
...
如果穿着护甲的人不想引人注目，隐藏护甲可以穿在衣服内。穿着隐藏护甲不是用`表：隐藏武器与物品`里的调整值，而是他人进行侦察检定（DC30）以发现护甲。
