...
任何听到密信的人都可以做一次察言观色检定（DC等于传递者的唬骗检定结果）。如果成功，偷听者会意识到交流中包含的密信。如果成功结果成功大于等于5点，偷听者将会破译密信。
...
|15|有足够抓住的把手点和脚踏点（天然或人工）的表面，比如粗糙的天然岩石表面、一棵树、或者铁丝网栅栏。单条没绳结的绳索。仅靠手悬挂着引体向上。|
...
如果专注检定成功，角色能如常进行该动作而不会引发任何借机攻击。如果专注检定失败，该行为的检定也会自动失败，视为角色的行为因分心而被扰乱了。但角色仍然不会引发借机攻击。
...
**此技能包括几个类别，每个类别被视为独立的技能：** 制造（化学）、制造（电学）、制造（机械）、制造（配药）、制造（结构）、制造（视觉艺术）、制造（写作）/
...
若检定成功，最终产品是一个固态的，砖头大小的爆炸化合物。爆炸化合物不包括导火索或起爆器。连接导火索或起爆器需要爆破检定。

|土制爆炸物类型|购买DC|制造DC|反射DC（半伤豁免所需）|时间|
|-------------------------------|:---------:|:------:|:------------------------------:|:--:|
|临时（1d6/5尺）\*|6|10|10|1 round|
...
**再次伤害：** 第二次豁免失败，角色暴露在毒素下一分钟后所受伤害。属性伤害是暂时的，除非标了星号，标星号的伤害是永久的`属性吸取`。昏迷持续1d3小时，麻痹持续2d6分钟。
...
|--------------------------------------------|:---------:|:------:|:--:|
|简单（计时器或起爆器）|8|15|1 小时|
|中等（无线电测向仪、电子锁）|12|20|12 小时|
|复杂（手机）|16|25|24 小时|
|高级（电脑）|22|30|60 小时|

**特别：** 无电气工具的角色进行制造（电子）检定将受到4点减值。

有`构造师`专长的角色制造（电子）检定有2点加值。
...
**检定：** 安装简单爆炸物爆破指定的位置不需要检定，但连接和安装起爆器需要。而且，将爆炸物放置在结构的最佳的破坏位置需要此检定，拆除爆破装置也需要。

**安装起爆器：** 大多数爆炸物都需要起爆器来引爆。连接起爆器到爆炸物需要一次爆破检定（DC10）。失败则意味着爆炸物无法按计划爆炸。失败10点或更多则意味着爆炸物在安装起爆器的瞬间爆炸。

角色可以让爆炸物难以拆除。即让角色在进行安装起爆器检定前，选择拆除DC（必须大于10）。角色安装起爆器的DC等于拆除的DC。
...
**拆除爆炸装置：** 拆除一个已安装并即将爆炸的爆炸物需要进行爆破检定。DC通常为10，除非安装起爆器的人设置了更高的拆除DC。若角色检定失败，将不能拆除爆炸物。若角色失败5点以上，爆炸物立即爆炸。
...
**时间：** 安装起爆器通常是一个整轮动作。安装一个爆炸装置需要一分钟及以上的时间，取决于工程量。
...
在正确的情境下，提供金钱或者其他形式的帮助可以提高角色的交涉技能检定成功率。角色可以用贿赂绕开各种官方的阻碍，只要受贿的人信任你或者有权限的人有意愿收受贿赂。
