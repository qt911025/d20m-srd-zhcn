...
**效果：** 近战时，每次角色若因目标隐藏而失手，可以重掷一次失手几率以查看自己是否真的命中。
...
**正常：** 没有寓守于攻专长的角色可以在攻击动作或全力攻击动作时采取防御式战斗，以受到4点攻击骰减值为代价在防御时获得2点闪避加值。
...
**效果：** 当角色使用此专长时，10尺内所有等级低于角色的对手必须进行一次意志豁免（DC为10+角色等级的一半+角色的魅力调整值）。豁免失败的对手将陷入`战栗`，所有攻击骰、豁免和技能检定有2点减值，持续轮数为1d6+角色魅力调整值。角色每轮可以以一个即时动作使用此专长。
...
**效果：** 近战时，若对手攻击角色失手，角色可以立即对此对手进行绊摔攻击。这算作一次借机攻击，角色徒手也能进行的借机攻击。这种绊摔攻击即使是徒手发动的，也不会引发借机攻击。
